...
#define ftdi_error_return(code, str) do {  \
	__android_log_print(ANDROID_LOG_INFO, "libftdi", "%s", str); \
        if ( ftdi )                        \
            ftdi->error_str = str;         \
        else                               \
            fprintf(stderr, str);          \
...
int ftdi_usb_open_dev2(struct ftdi_context *ftdi, libusb_device *dev, int fd)
{
    struct libusb_device_descriptor desc;
    struct libusb_config_descriptor *config0;
    int cfg, cfg0, detach_errno = 0;
...
int ftdi_usb_reset(struct ftdi_context *ftdi)
{
    ...
    int r;
    if ((r = libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                                SIO_RESET_REQUEST, SIO_RESET_SIO,
                                ftdi->index, NULL, 0, ftdi->usb_write_timeout)) < 0) {
        __android_log_print(ANDROID_LOG_INFO, "libftdi", " %d", r);
