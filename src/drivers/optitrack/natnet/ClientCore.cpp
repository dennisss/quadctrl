...
void ClientCore::start(){
	...
	inet_pton(AF_INET, "10.42.0.1", &mreq.imr_interface.s_addr);
...
void *data_server(void *arg){
	...
	while(cc->running){
		...
		// TODO: Call the callback here (if the data contained an sFrameOfMocapData
	}
	...
	*/
	...
	// Receive data packets and parse them
}
...
void ClientCore::unpackFrame(char *ptr){
	...
	sFrameOfMocapData *frame = (sFrameOfMocapData *)malloc(sizeof(sFrameOfMocapData));
	...
	for(int i = 0; i < nMarkerSets; i++){
		...
		set.Markers = (MarkerData *)malloc(sizeof(MarkerData) * nMarkers);
	...
	frame->OtherMarkers = (MarkerData *)malloc(sizeof(MarkerData) * nOtherMarkers);
	...
	for(int j = 0; j < nRigidBodies; j++){
		...
		MarkerData* markerData = (MarkerData *)malloc(nBytes);
		...
		if(major >= 2){
			...
			int32_t* markerIDs = (int32_t*)malloc(nBytes);
